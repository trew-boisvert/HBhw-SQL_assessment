...
SELECT id FROM orders WHERE customer_id = (SELECT id FROM customers WHERE fname LIKE 'Jane' AND lname LIKE 'Doe');
...
SELECT cupcakes.name, SUM(orders.num_cupcakes) FROM cupcakes LEFT JOIN orders ON cupcakes.id = orders.cupcake_id GROUP BY name ORDER BY name ASC;
...
SELECT customers.email FROM customers JOIN orders ON customers.id = orders.customer_id GROUP BY email HAVING SUM(num_cupcakes) > 100 ORDER BY email DESC;
